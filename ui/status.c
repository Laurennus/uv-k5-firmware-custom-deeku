...
void UI_DisplayStatus()
{
    ...
    // Power Save indicator
        ...
        UI_PrintStringSmallBufferNormal("PS", line + x);
    }
    x += 8;
#else
    // Power Save indicator
        ...
        if(!SCANNER_IsScanning()) {
        ...
        #endif
            {
                ...
                if(gEeprom.MENU_LOCK == true) {
                    UI_PrintStringSmallBufferNormal("RO", line + x + 2);
                }
                else
                {
                #endif
                    ...
                    if(dw == 1 || dw == 3) { // DWR - dual watch + respond
                        if(!gDualWatchActive)
                            UI_PrintStringSmallBufferNormal("><", line + x);
                        else if (dw==1)
                            UI_PrintStringSmallBufferNormal("DWR", line + x);
                        else if (dw==3)
                            UI_PrintStringSmallBufferNormal("DW", line + x);
                    }
                    else if(dw == 2) { // XB - crossband
                        UI_PrintStringSmallBufferNormal("XB", line + x);
                    }
                    else
                    {
                        UI_PrintStringSmallBufferNormal("MO", line + x);
                    }
                #ifdef ENABLE_FEAT_F4HWN_RESCUE_OPS
                }
                #endif
    ...
    if (gSetting_set_ptt_session) {
        UI_PrintStringSmallBufferNormal("OP", line + x);
    }
    else
    {
        UI_PrintStringSmallBufferNormal("CL", line + x);
    }
    x += 18;
#endif
    ...
    if (gEeprom.KEY_LOCK) {
        ...
        if (!gEeprom.MENU_LOCK) {
            UI_PrintStringSmallBufferNormal("F", line + x);
        }
        #else
            UI_PrintStringSmallBufferNormal("F", line + x);
        #endif
